...
final class Lemonway
{
                ...
                if (isset($_POST['iban_id']) && is_array($_POST['iban_id']) && check_admin_referer('moneyout_' . $walletId)) {
                    ...
                    try {
                        $params = array(
                            ...
                            "message" => __("Moneyout from Wordpress Woocommerce - Lemon Way for E-commerce module", LEMONWAY_TEXT_DOMAIN),
                            "ibanId" => $ibanId,
                        );
        ...
        <form method="post" action="">
            ...
            <div class="card iban-info">
                ...
                <?php if (count($wallet->ibans)) : ?>
                    <table>
                        <tr>
                            <td colspan="2"><?php echo __('Select an Iban', LEMONWAY_TEXT_DOMAIN) ?></td>
                        </tr>
                        <?php foreach ($wallet->ibans as $_iban) : /** @var $_iban Iban */ ?>
                            <tr>
                                <td>
                                    <input type="hidden" value="<?php echo $_iban->IBAN ?>"
                                           name="iban_<?php echo $_iban->ID ?>"/>
                                </td>
                                <td class="a-left">
                                    <label for="iban_<?php echo $_iban->ID ?>">
                                        <input class="required-entry" id="iban_<?php echo $_iban->ID ?>" type="radio"
                                               name="iban_id[]" value="<?php echo $_iban->ID ?>"/>
                ...
                <?php else: ?>
                    <div class="box">
                        ...
                        <?php echo sprintf(__('Please create at least one for wallet <b>%s</b> in <a href="%s">Lemonway BO </a>.', LEMONWAY_TEXT_DOMAIN), esc_html($wallet->ID), "https://ecommerce.lemonway.com/") ?>
                    </div>
                <?php endif; ?>
            </div>
                ...
                <div class="card moneyout-form">
                    ...
                    <table class="form-table">
                        ...
                        <tr>
                            ...
                            <td>
                                <input type="text" id="amountToPay" class="input-text not-negative-amount"
                                       name="amountToPay" value="<?php echo esc_attr($wallet->BAL) ?>">
                            </td>
                        </tr>
                        </tbody>
                    </table>
    ...
    function lw_install()
    {
        ...
        $result = $wpdb->update($wpdb->prefix . 'options', array('merchant_id' => 'bonjour', array('%s')));
        //var_dump($result);
